...
main(int argc, char **argv)
{
	...
	printf("File to receive from the server \n");
	printf("Press Q to quit \n");

	n=read(0, buff, BUFSIZE);	/* get user message */
	...
	while (1){
		...
		receivedBytes = read(s, (char *)&buff, BUFSIZE) - 1;
		...
		/* if data sent is the file name */
			...
			memmove(buff, buff+1, strlen(buff));
			packetNumber++;
			//receivedBytes = sizeof(buff) - 1;
			totalBytes += receivedBytes;
			fwrite(buff, 1,receivedBytes,fp);
